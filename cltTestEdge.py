...
class Test(DTCUT_test):
  ...
  tests_done   = None;
    ...
    # self.T.L has format ( [ (STRING protein1, STRING protein2) for all edges in STRING-db], [ STRING protein score] for all nodes in STRING-db])
  edge_nodes  = None;
  node_scores = None;
  ...
  def prepare_data(self):
    ...
    self.edge_nodes  = self.T.L[0];
    self.node_scores = self.T.L[1];
  #edef
  ...
  def score(self, i_node):
    ...
    nodes   = np.array(list(set([ self.edge_nodes[l][0] for l in leaves ] + [ self.edge_nodes[k][1] for l in leaves ])));
    return np.sum(self.node_scores[nodes]);
  #edef
